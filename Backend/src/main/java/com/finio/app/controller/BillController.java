...
public class BillController {
    ...
    @GetMapping
    public ResponseEntity<List<BillResponse>> getAll(@RequestParam(required = false) BillStatus status,
                                                     @AuthenticationPrincipal User user) {
        if (status != null) return ResponseEntity.ok(billService.getByStatus(user, status));
